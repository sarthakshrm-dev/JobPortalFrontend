const { validateRecruiterProfile } = require("../../utils/validators");
...
class RecruiterController {
  ...
  saveDetails = async (req, res, next) => {
    ...
    const { _id, __v, ...rest } = profile.toJSON();
    const [validatedData, err] = validateRecruiterProfile(data);
    if (err && err.length > 0) {
      return res.status(400).json({ error: err });
    }
    try {
      let result = await Recruiter.findByIdAndUpdate(
        { _id: _id },
        { $set: validatedData },
        { new: true }
      ).lean();
