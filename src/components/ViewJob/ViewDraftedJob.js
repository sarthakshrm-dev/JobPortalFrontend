...
export default function ViewDraftedJob({
  status,
  user,
  state,
  data,
  ...
  const { draftJobLoading, draftJobSuccess, draftJobError } =
    state.employerJobs;
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `/api/employer/job?status=${status}&page=${page}&limit=${itemsPerPage}`,
        {
          headers: {
  ...
  return (
    <>
      <h3 className="font-weight-bolder mb-3 text-center">Drafts</h3>
      {draftJobLoading ? (
        <Loading />
      ) : (
        <>
