...
import { setFetchClosedJobLoading, setFetchClosedJobSuccess, setFetchClosedJobError, setUnfavouriteJobError, setUnfavouriteJobLoading, setUnfavouriteJobSuccess } from "../../state/jobseekerJob/jobseekerJobSlice";
...
export default function ViewClosedJob({
  ...
  const { fetchClosedJobLoading, fetchClosedJobSuccess, fetchClosedJobError, unfavouriteJobLoading, unfavouriteJobSuccess, unfavouriteJobError } = state.jobseekerJobs;

  const navigate = useNavigate();

  useEffect(() => {
    navigate(`/dashboard/jobs/closed?page=${page || 1}`);
    dispatch(setFetchClosedJobLoading());
    axios
      .get(
      ...
      .then((res) => {
        ...
        setData(jobs)
        dispatch(setFetchClosedJobSuccess());
      })
      .catch((err) => {
        console.log(err);
        dispatch(setFetchClosedJobError(err));
        setData([]);
      });
